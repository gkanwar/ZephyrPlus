...
def send_zephyr(cls, instance, recipient, sender, message, signature):
    ...
    z = zephyr.ZNotice(cls=encode(cls),
...
def _check_hmac(z):
    ...
    with _seen_times_lock:
        ...
        if random.random() < 0.001 and current_time - min(_seen_times) > CLOCK_SKEW_MAX_S:
            _seen_times = {t for t in _seen_times if current_time - t < CLOCK_SKEW_MAX_S}
